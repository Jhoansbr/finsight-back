  ...
  body('categorias')
    ...
    .custom((categorias) => {
      ...
      for (const cat of categorias) {
        if (!cat.categoriaId || !Number.isInteger(cat.categoriaId)) {
          throw new Error('Cada categoría debe tener un ID válido');
        }
        if (!cat.montoAsignado || isNaN(cat.montoAsignado) || cat.montoAsignado <= 0) {
          throw new Error('El monto asignado debe ser un número positivo');
        }
        if (ids.has(cat.categoriaId)) {
          throw new Error('No puede haber categorías duplicadas');
        }
        ids.add(cat.categoriaId);
      }
      return true;
    }),
];
